...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r"\b(?:[01]?\d|2[0-3]):[0-5]\d[APap]?[Mm]?\b", text)
...
def most_common_code(text):
    ...
    '123'
    """
    codes_lst = area_codes(text)
    running_high_count = 0
    common_code = ''
    for area_code in codes_lst:
        code_freq = codes_lst.count(area_code)
        if code_freq > running_high_count:
            running_high_count = code_freq
            common_code = area_code
    return common_code
