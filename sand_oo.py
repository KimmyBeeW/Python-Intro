...
class Sand:
    ...
    def is_move_ok(self, x_to, y_to):
        if self.grid.in_bounds(x_to, y_to) and self.grid.get(x_to, y_to) is None:
            return False if (((x_to == self.x + 1 or x_to == self.x - 1) and y_to == self.y + 1)
                             and self.grid.get(x_to, self.y) is not None) else True
        else:
            return False
    ...
    def move(self, physics):
        ...
        if cor is None:
            ...
            self.x = x_to
            self.y = y_to
            self.grid.set(self.x, self.y, Sand(self.grid, self.x, self.y))
