...
def add_object(grid: Grid, object_type, x: int, y: int):
    """
    ...
    :param grid: a reference to the grid where you want to add the object to
    :param object_type: a reference to the class of the particle to be added (so either Sand, Rock, or Bubble)
    :param x: the x coordinate of the new object
    :param y: the y coordinate of the new object
    """
    if grid.get(x, y) is not None:  # check grid location is clear
        return  # exit
    else:
        obj = object_type(grid, x, y)  # create object w/ x and y coord.s
        all_grid_objects.append(obj)  # store in list
        grid.set(x, y, obj)  # store in grid
    #     return obj
    # return None
...
def remove_object(grid, x, y):
    ...
    """
    obj = grid.get(x, y)
    if obj is None:  # verify particle in pos.
        return
    grid.set(x, y, None)  # remove from grid
    all_grid_objects.remove(obj)  # remove from list
...
def do_whole_grid():
    ...
    (4) reverse order of the list one last time because it will make sorting the list next time just that much easier
    """
