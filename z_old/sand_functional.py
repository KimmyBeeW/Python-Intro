...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    Doctests: # python3 -m doctest sand_functional.py
    >>> grid2 = Grid.build([[None, 's',   'r'], [None, None, None]])
    >>> is_move_ok(grid2, 1, 0, 1, 1)  # down ok
    True
    >>> is_move_ok(grid2, 1, 0, 0, 0)  # left ok
    True
    >>> is_move_ok(grid2, 1, 0, 2, 0)  # right ok
    False
    >>> grid3 = Grid.build([[None, 's',   'r'], [None, "r", None]])
    >>> is_move_ok(grid3, 1, 0, 1, 1)  # down ok
    False
    >>> is_move_ok(grid3, 1, 0, 0, 1)  # down corner left
    True
    >>> is_move_ok(grid3, 1, 0, 2, 1)  # down corner right
    False
    """
    if grid.in_bounds(x_to, y_to) and grid.get(x_to, y_to) is None:
        return False if (((x_to == x_from+1 or x_to == x_from-1) and y_to == y_from+1)
                         and grid.get(x_to, y_from) is not None) else True
    else:
        return False
...
def do_gravity(grid, x, y):
    ...
    Doctests: # python3 -m doctest sand_functional.py
    >>> grid2 = Grid.build([[None, 's', None], [None, None, None]]) # not sand
    >>> do_gravity(grid2, 0, 0)
    [[None, 's', None], [None, None, None]]
    >>> grid3 = Grid.build([[None, 's', None], [None, None, None]]) # down
    >>> do_gravity(grid3, 1, 0)
    [[None, None, None], [None, 's', None]]
    >>> grid4 = Grid.build([[None, 's', None], [None, 'r', None]]) # down left
    >>> do_gravity(grid4, 1, 0)
    [[None, None, None], ['s', 'r', None]]
    >>> grid5 = Grid.build([[None, 's', None], ['s', 'r', None]]) # down right
    >>> do_gravity(grid5, 1, 0)
    [[None, None, None], ['s', 'r', 's']]
    >>> grid6 = Grid.build([['r', 's', None], [None, 'r', None]]) # corner rule
    >>> do_gravity(grid6, 1, 0)
    [['r', None, None], [None, 'r', 's']]
    >>> grid7 = Grid.build([['r', 's', 'r'], [None, 'r', None]]) # double corner rule
    >>> do_gravity(grid7, 1, 0)
    [['r', 's', 'r'], [None, 'r', None]]
    >>> grid8 = Grid.build([['s', 's', 's'], [None, 'r', None]]) # lots of sand
    >>> do_gravity(grid8, 1, 0)
    [['s', 's', 's'], [None, 'r', None]]
    """
    if grid.get(x, y) == 's':
        grid1 = deepcopy(grid)
        if is_move_ok(grid1, x, y, x, y+1):
            grid1 = do_move(grid1, x, y, x, y+1)
        elif is_move_ok(grid1, x, y, x-1, y+1):
            grid1 = do_move(grid1, x, y, x-1, y+1)
        elif is_move_ok(grid1, x, y, x+1, y+1):
            grid1 = do_move(grid1, x, y, x+1, y+1)
        return grid1.array
    else:
...
def do_whole_grid(grid):
    ...
    Doctests: # python3 -m doctest sand_functional.py
    >>> grid3 = Grid.build([['s', 's', 's'], [None, None, None], [None, None, None]])
    >>> do_whole_grid(grid3)
    Grid.build([[None, None, None], ['s', 's', 's'], [None, None, None]])
    >>> grid4 = Grid.build([[None, None, None], ['s', 's', 's'], [None, None, None]])
    >>> do_whole_grid(grid4)
    Grid.build([[None, None, None], [None, None, None], ['s', 's', 's']])
    """
    grid1 = deepcopy(grid)
    for y in reversed(range(grid.height)):
        for x in range(grid.width):
            grid2 = Grid.build(do_gravity(grid1, x, y))
            while grid1 != grid2:
                grid1 = grid2
                grid2 = Grid.build(do_gravity(grid1, x, y))
    return grid1
